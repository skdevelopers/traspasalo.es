<nav class="bg-violet-950 border-violet-900 dark:bg-gray-900 px-4 md:px-20">
    <div class="container xl:container-xl px-0 flex flex-wrap items-center justify-between mx-auto py-2">
        ...
        <a href="{{ url('/') }}" class="flex items-center space-x-5 md:space-x-0 rtl:space-x-reverse">
            <img src="{{ asset('/front/assets/images/logo-white.png') }}" class="h-10 md:h-12 xl:h-16" alt="{{ translate('Logo') }}" />
        </a>
                ...
                <div id="languageDropdownMenu"
                    ...
                    <div class="py-1" role="none">
                        <a href="{{ url('lang/es') }}"
                            ...
                            role="menuitem">{{ translate('Español') }}</a>
                        <a href="{{ url('lang/en') }}"
                            class="text-gray-700 block px-4 py-2 text-sm hover:bg-violet-600 hover:text-white"
                            role="menuitem">{{ translate('English') }}</a>
                        <a href="{{ url('lang/ca') }}"
                            class="text-gray-700 block px-4 py-2 text-sm hover:bg-violet-600 hover:text-white"
                            role="menuitem">{{ translate('Catalan') }}</a>
                    </div>
                </div>
            </div>
                    ...
                    <div id="authDropdownMenu"
                        ...
                        <div class="py-1">
                            <a href="{{ route('register.step1') }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-violet-600 hover:text-white">{{ translate('Register') }}</a>
                            <a href="{{ route('auth.login') }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-violet-600 hover:text-white">{{ translate('Login') }}</a>
                        </div>
                    </div>
                </div>
            @endguest
                ...
                <div class="relative">
                    ...
                    <div
                        ...
                        <a class="flex items-center py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300"
                            ...
                            <span>
                                @if (Auth::check())
                                    <p>{{ translate('Hi') }}, {{ Auth::user()->first_name }}</p>
                                @else
                                    <p>{{ translate('Welcome') }}, {{ translate('Guest') }}</p>
                                @endif
                            </span>
                        </a>
                        <a href="{{ route('home') }}"
        ...
        <div class="hidden w-full lg:flex lg:w-auto items-center" id="navbar-default">
            <ul
                ...
                <li>
                    <a href="{{ url('/about') }}"
                        class="block py-2 px-3 hover:bg-violet-600 lg:hover:bg-transparent lg:hover:text-violet-400 rounded">{{ translate('About Us') }}</a>
                </li>
                <li>
                    <a href="{{ url('/newsletter') }}"
                        class="block py-2 px-3 hover:bg-violet-600 lg:hover:bg-transparent lg:hover:text-violet-400 rounded">{{ translate('Newsletter') }}</a>
                </li>
                <li>
                    <a href="{{ url('/price') }}"
                        class="block py-2 px-3 hover:bg-violet-600 lg:hover:bg-transparent lg:hover:text-violet-400 rounded">{{ translate('Pricing') }}</a>
                </li>
                <li>
                    <a href="{{ url('/blog') }}"
                        class="block py-2 px-3 hover:bg-violet-600 lg:hover:bg-transparent lg:hover:text-violet-400 rounded">{{ translate('Blogs') }}</a>
                </li>
                <li class="block lg:hidden">
                    <a href="{{ url('/add-business') }}"
                        class="inline-block text-gray-300 border-2 border-gray-300 rounded px-6 py-2 hover:bg-gray-300 hover:text-violet-950">
                        {{ translate('Add Business') }}</a>
                </li>
            </ul>
            ...
            <div class="hidden lg:flex justify-center items-center space-x-3 p-3">
                <a href="{{ url('/add-business') }}"
                    class="text-gray-300 border-2 border-gray-300 rounded px-6 py-2 hover:bg-gray-300 hover:text-violet-950">{{ translate('Add Business') }}</a>
                ...
                <div class="relative flex place-content-center place-items-center text-center">
                    ...
                    <div id="languageDropdownMenu1"
                        class="hidden origin-top-right absolute right-4 mt-36 w-24 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50"
                        role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                        <div class="py-1" role="none">
                            <a href="{{ url('lang/es') }}"
                                class="text-gray-700 block px-4 py-2 text-sm hover:bg-violet-600 hover:text-white"
                ...
                @guest
                    <div class="relative inline-block text-left">
                        ...
                        <div id="authDropdownMenu1"
                            ...
                            <div class="py-1">
                                <a href="{{ route('register.step1') }}"
                                    class="block px-4 py-2 text-sm text-gray-700 hover:text-white hover:bg-violet-600">{{ translate('Register') }}</a>
                                <a href="{{ route('auth.login') }}"
                                    class="block px-4 py-2 text-sm text-gray-700 hover:text-white hover:bg-violet-600">{{ translate('Login') }}</a>
                            </div>
                        </div>
                    </div>
                @endguest
