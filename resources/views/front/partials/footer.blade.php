    ...
    <div class="container xl:container-xl mx-auto px-4">
        <div class="lg:flex lg:justify-between ">
            <div class="lg:py-6 lg:w-1/2 w-full">
                <h2 class="text-2xl md:text-4xl mb-0 md:mb-4 text-gray-100 lg:border-r-2 lg:pr-32 md:pr-20 pr-5">{{ translate('Let\'s Start to find Best Business') }}</h2>
            </div>
            <div class="mb-6 lg:mb-0 p-0 lg:p-2 lg:w-1/2 w-full lg:flex lg:justify-end">
                <div>
                    <h3 class="text-lg md:mb-0 md:mr-4 text-gray-300 lg:pb-3">{{ translate('Join Our Newsletter') }}</h3>
                    <form id="subscribeForm" class="flex flex-col md:flex-row items-start md:items-center space-y-2 lg:space-y-0">
                        @csrf
                        <input type="email" name="email" id="email" placeholder="{{ translate('Your Email') }}" class="w-full md:w-80 p-2 rounded-sm focus:outline-none text-gray-800 md:mr-4">
                        <button type="submit" class="bg-orange-500 rounded-sm px-4 py-2  md:ml-5 lg:ml-0">{{ translate('Subscribe') }}</button>
                    </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mt-7">
            ...
            <div class="col-span-1 md:col-span-4 lg:col-span-5 md:pr-16 lg:pr-16">
                <div class="relative">
                    <img src="{{ asset('front/assets/images/footer-logo.svg') }}" alt="{{ translate('Logo') }}" class="mb-4 w-10 h-12">
                    <p class="text-sm">{{ translate('Nullo eleifend pulvinar purus, malesuada euismod odio imperdiet. Ut sit amet erat nec nibh rhoncus varius in non lorem. Donec interdum, lectus in convallis.') }}</p>
                </div>
            </div>
            ...
            <div class="col-span-1 md:col-span-2 lg:col-span-2">
                <div class="flex md:justify-center justify-start">
                    <ul class="lg:pl-0 pl-0">
                        <li class="mb-2">{{ translate('Countries') }}</li>
                        <li class="mb-2">{{ translate('Regions') }}</li>
                        <li class="mb-2">{{ translate('Cities') }}</li>
                        <li class="mb-2">{{ translate('Districts') }}</li>
                        <li class="mb-2">{{ translate('Hotels') }}</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="col-span-1 md:col-span-6 lg:col-span-5">
                <div class="flex flex-col md:flex-row justify-end space-y-4 md:space-y-0 md:space-x-8">
                    <div class="relative">
                        <ul>
                            <li class="mb-2">{{ translate('Unique places to stay') }}</li>
                            <li class="mb-2">{{ translate('Reviews') }}</li>
                            <li class="mb-2">{{ translate('Discover monthly stays') }}</li>
                            <li class="mb-2">{{ translate('Unpacked: Travel articles') }}</li>
                            <li class="mb-2">{{ translate('Seasonal and holiday deals') }}</li>
                            <li class="mb-2">{{ translate('Traveller Review Awards') }}</li>
                        </ul>
                    </div>
                    <div class="relative">
                        <ul>
                            <li class="mb-2">{{ translate('About Us') }}</li>
                            <li class="mb-2">{{ translate('Customer Service Help') }}</li>
                            <li class="mb-2">{{ translate('Partner help') }}</li>
                            <li class="mb-2">{{ translate('Careers') }}</li>
                            <li class="mb-2">{{ translate('Sustainability') }}</li>
                            <li class="mb-2">{{ translate('Terms & Conditions') }}</li>
                            <li class="mb-2">{{ translate('How We Work') }}</li>
                            <li class="mb-2">{{ translate('Privacy & cookie statement') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
