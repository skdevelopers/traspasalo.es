@extends('layouts.vertical', ['title' => 'Edit Translations', 'sub_title' => 'Translate Words'])
    ...
    <div class="grid grid-cols-12 gap-4">
        ...
        <div class="col-span-12">
            <div class="bg-white rounded-md shadow-md p-6">
                <h1 class="text-2xl font-bold mb-6">Edit Translation</h1>

                <form action="{{ route('translations.update', $translation->id) }}" method="POST" class="grid grid-cols-12 gap-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-span-12">
                        ...
                        <input type="text" name="key" id="key" value="{{ $translation->key }}" required 
                               ...
                               @error('key')
                               <span class="text-red-600">Key Duplication</span>
                           @enderror
                            </div>

                    <div class="col-span-12">
                        <label for="language" class="block text-sm font-medium text-gray-700">Language</label>
                        <input type="text" name="language" id="language" value="{{ $translation->language }}" required 
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    ...
                    <div class="col-span-12">
                        ...
                        <textarea name="value" id="value" required rows="4" 
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ $translation->value }}</textarea>
                    </div>
